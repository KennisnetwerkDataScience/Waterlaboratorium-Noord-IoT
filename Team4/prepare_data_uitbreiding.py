...
# change these
file_location = Path(r'D:\Files\wnl_data')
output_file_location  = Path(r'D:\Files\wnl_data\output')

# parse a string that looks like this '23/ 534/ 344' and calculate the mean
def mean_value_str(split_char, x_str):    
    return mean([float(value.strip()) for value in x_str.split(split_char)])
...
def prepare_data(d_sheets):
    ...
    for sheet_name, df_sheet in d_sheets.items():
        for column in df_sheet.columns:
            
            if df_sheet[column].dtype == 'O':
                #print("sheet: {0} column: {1} to numeric".format(sheet_name, column))
    
                # replace values that are not numeric (like <3 or 36/ 37)
...
sheets_kwaliteits_data = read_in_all_sheets(file_location / 'uitbreiding_Kennisnetwerk_procesdata_kwaliteitsdata_WLN_2010-2018.xlsx')
